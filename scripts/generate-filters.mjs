...
const pixiFilters = [
  ...
  ['ReflectionFilter', 'reflection', { mirror: true, boundary: 0.5, amplitude: 0, waveLength: 30, alpha: 0.5 }, [['mirror', 'boolean'], ['boundary', 'number', 0, 1], ['amplitude', 'number', 0, 40], ['waveLength', 'number', 1, 120], ['alpha', 'number', 0, 1]], "if (uv.y > uBoundary) { vec2 ruv = vec2(uv.x + sin((uv.y + uTime * 0.01) * uWaveLength) * uAmplitude / max(uResolution.x, 1.0), uBoundary - (uv.y - uBoundary)); color = readInput(ruv); color.a *= uAlpha * (1.0 - uv.y); } gl_FragColor = color;"],
