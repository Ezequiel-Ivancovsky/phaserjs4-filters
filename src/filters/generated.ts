...
const FILTER_BODIES = {
  ...
  ReflectionFilter: "if (uv.y > uBoundary) { vec2 ruv = vec2(uv.x + sin((uv.y + uTime * 0.01) * uWaveLength) * uAmplitude / max(uResolution.x, 1.0), uBoundary - (uv.y - uBoundary)); color = readInput(ruv); color.a *= uAlpha * (1.0 - uv.y); } gl_FragColor = color;",
...
const FILTER_UNIFORMS = {
  ...
  ReflectionFilter: "uniform float uMirror;\nuniform float uBoundary;\nuniform float uAmplitude;\nuniform float uWaveLength;\nuniform float uAlpha;",
...
export const FILTER_METADATA = ([
  ...
  },
  {
    ...
    source: 'pixi-source',
    fishOnly: false,
    defaults: {"mirror":true,"boundary":0.5,"amplitude":0,"waveLength":30,"alpha":0.5},
    controls: [{"key":"mirror","type":"boolean"},{"key":"boundary","type":"number","min":0,"max":1},{"key":"amplitude","type":"number","min":0,"max":40},{"key":"waveLength","type":"number","min":1,"max":120},{"key":"alpha","type":"number","min":0,"max":1}]
  },
  {
    id: 'RGBSplitFilter',
